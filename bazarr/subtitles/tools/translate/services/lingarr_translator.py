...
from app.event_handler import hide_progress, show_message, show_progress
...
from ..core.translator_utils import (
    add_translator_info,
...
class LingarrTranslatorService:
    ...
    def translate(self):
        try:
            ...
            logger.debug(
                f"BAZARR saving Lingarr translated subtitles to {self.dest_srt_file}"
            )
            translation_map = {}
            ...
            try:
                ...
                logger.error(
                    f"BAZARR is unable to save translated subtitles to {self.dest_srt_file}"
                )
                show_message(
                    f"Translation failed: Unable to save translated subtitles to {self.dest_srt_file}"
                )
                raise OSError
            ...
            logger.error(
                f"BAZARR encountered an error during Lingarr translation: {str(e)}"
            )
    ...
    def _translate_content(self, lines_list):
        try:
            source_lang = self.language_code_convert_dict.get(
                self.from_lang, self.from_lang
            )
            target_lang = self.language_code_convert_dict.get(
                self.orig_to_lang, self.orig_to_lang
            )

            lines_payload = []
            for i, line in enumerate(lines_list):
                lines_payload.append({"position": i, "line": line})
            ...
            if self.media_type == "series":
                ...
                context = self._get_movie_context()
            ...
            if response.status_code == 200:
                ...
                if isinstance(translated_batch, list):
                    for item in translated_batch:
                        if (
                            ...
                            logger.error(
                                f"Invalid response format from Lingarr API: {item}"
                            )
                            return None
                    return translated_batch
                else:
                    logger.error(
                        f"Unexpected response format from Lingarr API: {translated_batch}"
                    )
                    return None
                ...
                logger.debug(
                    f"Lingarr API error: {response.status_code} - {response.text}"
                )
                return None
    ...
    def _get_episode_context(self):
        """Get episode and show context for better translation quality."""
        context = {}

        try:
            # Get show information
            if self.sonarr_series_id:
                show_row = database.execute(
                    select(
                        TableShows.title,
                        TableShows.overview,
                        TableShows.year,
                        TableShows.seriesType,
                    ).where(TableShows.sonarrSeriesId == self.sonarr_series_id)
                ).first()

                if show_row:
                    context["showTitle"] = getattr(show_row, "title", None)
                    context["showOverview"] = getattr(show_row, "overview", None)
                    context["showYear"] = getattr(show_row, "year", None)
                    context["seriesType"] = getattr(show_row, "seriesType", None)
            ...
            if self.sonarr_episode_id:
                episode_row = database.execute(
                    select(
                        ...
                        TableEpisodes.title,
                        TableEpisodes.overview,
                    ).where(TableEpisodes.sonarrEpisodeId == self.sonarr_episode_id)
                ).first()

                if episode_row:
                    context["season"] = getattr(episode_row, "season", None)
                    context["episode"] = getattr(episode_row, "episode", None)
                    context["episodeTitle"] = getattr(episode_row, "title", None)
                    context["episodeOverview"] = getattr(episode_row, "overview", None)

        except Exception as e:
            logger.debug(f"Could not fetch episode context: {str(e)}")

        return context

    def _get_movie_context(self):
        """Get movie context for better translation quality."""
        context = {}
        ...
        try:
            if self.radarr_id:
                movie_row = database.execute(
                    select(
                        TableMovies.title, TableMovies.overview, TableMovies.year
                    ).where(TableMovies.radarrId == self.radarr_id)
                ).first()

                if movie_row:
                    context["movieTitle"] = getattr(movie_row, "title", None)
                    context["overview"] = getattr(movie_row, "overview", None)
                    context["year"] = getattr(movie_row, "year", None)

        except Exception as e:
            logger.debug(f"Could not fetch movie context: {str(e)}")
